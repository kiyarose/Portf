      ...
      <div className="card-surface space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {certifications.map((cert, index) => (
            ...
            >
              <span className="chip !bg-accent/15 !text-accent dark:!bg-accent/20">{cert.issuer}</span>
              <h3 className="mt-4 text-xl font-semibold text-slate-900 dark:text-slate-50">{cert.name}</h3>
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">Issued {cert.date}</p>
            </motion.div>
          ))}
        </div>
      </div>
