...
const sections = [
  ...
  { id: 'education', label: 'Education', icon: 'material-symbols:school-rounded' },
  { id: 'certifications', label: 'Certifications', icon: 'material-symbols:workspace-premium-rounded' },
  { id: 'projects', label: 'Projects', icon: 'material-symbols:rocket-launch-rounded' },
  { id: 'skills', label: 'Skills', icon: 'material-symbols:sparkles-rounded' },
  { id: 'contact', label: 'Contact', icon: 'material-symbols:contact-mail-rounded' },
]
  ...
  return (
    <>
      <motion.div
        ...
        transition={{ repeat: prefersReducedMotion ? 0 : Infinity, duration: 18, ease: 'easeInOut' }}
      />
      <motion.div
        ...
        transition={{ repeat: prefersReducedMotion ? 0 : Infinity, duration: 12, ease: 'easeInOut' }}
      />
    </>
  )
}
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-white/75 backdrop-blur-xl transition dark:bg-slate-950/70">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-4 py-4">
          <a href="#hero" className="flex items-center gap-3 text-sm font-semibold uppercase tracking-[0.2em] text-slate-600 dark:text-slate-300">
            <Icon icon="material-symbols:apps-rounded" className="text-2xl text-accent" aria-hidden="true" />
            Kiya Rose
          </a>
      ...
      <footer className="border-t border-white/10 bg-white/50 py-6 text-center text-sm text-slate-500 backdrop-blur dark:bg-slate-950/70 dark:text-slate-400">
        © {new Date().getFullYear()} Kiya Rose. Crafted with React, Tailwind CSS, and Firebase.
      </footer>
    </div>
  )
}
