  ...
  while (args.length > 0) {
    ...
    switch (arg) {
      ...
      case "--pick": {
        const value = args.shift();
        if (!value) {
          throw new Error("--pick requires a comma-separated list of export names");
        }
        const picks = value
          .split(",")
          .map((item) => item.trim())
  ...
  if (pickSet) {
    for (const requested of pickSet) {
      if (!seenNames.has(requested)) {
        throw new Error(`Requested export \`${requested}\` was not found or is not convertible`);
      }
    }
  }
  ...
  switch (typeof value) {
    ...
    case "object":
      if (Array.isArray(value)) {
        ...
        return ts.factory.createArrayLiteralExpression(elements, elements.length > 0);
      }
      ...
      const entries = Object.entries(value).map(([key, item]) => {
        ...
        return ts.factory.createPropertyAssignment(nameNode, createLiteralFromValue(item));
      });

      return ts.factory.createObjectLiteralExpression(entries, entries.length > 0);
    default:
...
function formatValueForTs(value, baseIndent) {
  ...
  const printed = printer.printNode(ts.EmitHint.Expression, literal, printerSourceFile);
  ...
  for (let index = 1; index < lines.length; index += 1) {
    ...
    const indentLevel = leadingSpaces.length > 0 ? Math.floor(leadingSpaces.length / 4) : 0;
...
function convertTsToJson(filePath, { out, overwrite, picks, quiet }) {
  ...
  const output = { __meta: { version: VERSION, template, source: path.relative(process.cwd(), resolvedPath), entries: entries.map((entry) => ({
    name: entry.name,
...
function convertJsonToTs(filePath, { out, overwrite, picks, quiet }) {
  ...
  if (!parsed.__meta || typeof parsed.__meta !== "object") {
    throw new Error("Missing __meta section. This JSON file was not generated by data-converter.mjs");
  }
...
function main() {
  ...
  if (options.out && options.inputs.length > 1) {
    console.error("--out can only be used when processing a single input file.");
    process.exit(1);
  }
