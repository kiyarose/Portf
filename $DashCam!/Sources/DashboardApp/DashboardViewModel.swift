...
final class DashboardViewModel: ObservableObject {
  ...
  init(defaultPath: String? = nil) {
    ...
    self.isProjectDirectoryCollapsed =
      defaults.object(forKey: CollapseKey.project) as? Bool ?? false
  ...
  private func updateDiscordActivity() {
    ...
    if processCount == 0 {
      ...
      let details =
        processCount == 1
        ? "Running \(runningProcesses.first ?? "Unknown Tool")" : "Running \(processCount) tools"
...
final class GitController: ObservableObject {
    ...
    for command in commands {
      ...
      do {
        ...
        if result.exitCode != 0 {
          ...
          appendLog(
            "❌ git \(command.arguments.first ?? "command") exited with status \(result.exitCode)")
          return
        }
      } catch {
  ...
  private func executeGitCommand(_ arguments: [String], in directory: URL) async throws
    -> GitCommandResult
  {
