...
final class CodexController: ObservableObject {
    ...
    var collapsedSummary: String? {
        entries.reversed().first { $0.role == .assistant }?.text.trimmingCharacters(in: .whitespacesAndNewlines)
    }
    ...
    private func systemPrompt() -> String {
        var prompt = "You are Codex, an expert engineering assistant embedded in the DashCam! macOS dashboard. Provide concise, Markdown-formatted answers with actionable steps and code where it helps. If more detail is needed, ask clarifying questions first."
        if includeProjectContext, let summary = projectSummary {
            prompt += " The active project directory is \(summary). Reference files relative to this directory when suggesting edits."
        }
        prompt += " Prefer short command snippets (use fenced code blocks) and call out risky operations explicitly."
        return prompt
    }
    ...
    nonisolated private static func execute(prompt: String, context: CommandContext) async throws -> String {
