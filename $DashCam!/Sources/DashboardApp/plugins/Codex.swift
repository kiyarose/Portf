...
struct CodexMGM: View {
  ...
  private var canSend: Bool {
    !controller.isProcessing && controller.isCommandConfigured
    ...
    VStack(alignment: .leading, spacing: 14) {
      ...
      if !isCollapsed {
        transcript
          ...
          .animation(
            reduceMotion ? nil : .easeInOut(duration: 0.18), value: controller.entries.count)
        footer
  ...
  private var statusBadge: some View {
    ...
    case .idle:
      ...
      StatusBadge(
        text: configured ? "Ready" : "Command missing", color: configured ? .secondary : .red)
    case .sending:
  ...
  private var connectionControls: some View {
    DisclosureGroup(isExpanded: $showConnectionSettings) {
      VStack(alignment: .leading, spacing: 12) {
        ...
        Text(
          "Arguments are split using shell-style rules. Wrap paths or phrases in quotes when needed."
        )
        .font(.footnote)
        .foregroundStyle(.secondary)
...
private struct CodexTranscriptRow: View {
  ...
  var body: some View {
    ...
    case .user:
      HStack(alignment: .top) {
        Spacer(minLength: 60)
        bubble(
          title: "You", icon: "person.fill", background: Color.accentColor.opacity(0.16),
          textColor: .accentColor, alignment: .trailing)
      }
      .transition(.move(edge: .trailing).combined(with: .opacity))
    case .assistant:
      HStack(alignment: .top) {
        bubble(
          title: "Codex", icon: "sparkles", background: Color.primary.opacity(0.05),
          textColor: .primary, alignment: .leading)
        Spacer(minLength: 60)
      }
  ...
  @ViewBuilder
  private func bubble(
    title: String, icon: String, background: Color, textColor: Color, alignment: HorizontalAlignment
  ) -> some View {
